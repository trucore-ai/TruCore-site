...
import type { DemoLiveReceipt } from "@/lib/demo-live";
...
export function DemoLiveStream() {
  const [receipts, setReceipts] = useState<DemoLiveReceipt[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadDemoLive() {
      try {
        ...
        const payload = (await response.json()) as DemoLivePayload | { ok?: false; error?: string };
