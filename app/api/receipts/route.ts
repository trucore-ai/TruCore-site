/**
 * Customer receipts proxy route - list and verify.
 *
 * GET /api/receipts - list receipts (authenticated)
 * POST /api/receipts - verify receipt signature (authenticated)
 */
...
export async function GET(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`receipts:list:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const path = queryString ? `/customer/receipts?${queryString}` : "/customer/receipts";
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), path);
...
export async function POST(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`receipts:verify:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/customer/receipts/verify");
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), TIMEOUT_MS);
  ...
  try {
    ...
    return NextResponse.json(
      { error: "upstream_unavailable", message: "Receipt verification is temporarily unavailable." },
      { status: 502, headers: NO_STORE },
    );
  }
}
