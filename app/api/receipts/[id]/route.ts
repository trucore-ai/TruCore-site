/**
 * Customer receipt detail proxy route.
 *
 * GET /api/receipts/[id] - fetch single receipt (authenticated)
 */
...
export async function GET(
  ...
  const rl = consumeRateLimit(`receipts:detail:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(
    getAtfApiBaseUrl(),
    `/customer/receipts/${encodeURIComponent(id)}`,
  );
