/**
 * Customer upgrade request cancel proxy route.
 *
 * POST /api/customer/upgrades/[id]/cancel - cancel upgrade request (authenticated)
 */
...
export async function POST(
  ...
  const rl = consumeRateLimit(`customer:upgrades:cancel:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(
    getAtfApiBaseUrl(),
    `/customer/upgrades/${encodeURIComponent(id)}/cancel`,
  );
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/upgrades/cancel",
