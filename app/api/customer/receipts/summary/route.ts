...
const RATE_LIMIT_MAX = 10; // per IP per minute (lower — summary is heavier)
const NO_STORE = { "Cache-Control": "no-store" };

export async function GET(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`customer:receipts-summary:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstreamPath = `/customer/receipts/summary${queryString ? `?${queryString}` : ""}`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/receipts/summary",
    ...
    return NextResponse.json(
      {
        ...
        message: isTimeout
          ...
          : "Receipt summary service is temporarily unavailable.",
      },
