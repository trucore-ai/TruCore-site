...
export async function GET(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`customer:intel-benchmarks:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/customer/intel/benchmarks");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/intel/benchmarks",
    ...
    return NextResponse.json(
      {
        ...
        message: isTimeout
          ...
          : "Benchmark service is temporarily unavailable.",
      },
