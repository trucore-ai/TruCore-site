...
const RATE_LIMIT_MAX = 10; // per IP per minute (heavier than market-conditions)
const NO_STORE = { "Cache-Control": "no-store" };

export async function GET(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`customer:intel-recommendations:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/customer/intel/recommendations");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/intel/recommendations",
    ...
    return NextResponse.json(
      {
        ...
        message: isTimeout
          ...
          : "Policy intelligence service is temporarily unavailable.",
      },
