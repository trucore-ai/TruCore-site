...
export async function GET(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`customer:intel-external-context:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/customer/intel/external-context");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/intel/external-context",
    ...
    return NextResponse.json(
      {
        ...
        message: isTimeout
          ...
          : "External context service is temporarily unavailable.",
      },
