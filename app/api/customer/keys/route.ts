/**
 * Customer API keys proxy route - list and create.
 *
 * GET /api/customer/keys - list keys (authenticated)
 * POST /api/customer/keys - create key (authenticated)
 */
...
const RATE_LIMIT_MAX_CREATE = 10;
const MAX_BODY_BYTES = 4 * 1024;
const NO_STORE = { "Cache-Control": "no-store" };

export async function GET(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`customer:keys:list:${sha256(ip)}`, {
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/customer/keys");
...
export async function POST(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`customer:keys:create:${sha256(ip)}`, {
