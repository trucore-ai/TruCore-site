/**
 * Customer API key rotate proxy route.
 *
 * POST /api/customer/keys/[id]/rotate - rotate key (authenticated)
 */
...
export async function POST(
  ...
  const rl = consumeRateLimit(`customer:keys:rotate:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(
    getAtfApiBaseUrl(),
    `/customer/keys/${encodeURIComponent(id)}/rotate`,
  );
