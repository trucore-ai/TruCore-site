...
const RATE_LIMIT_MAX = 15; // per IP per minute (lightweight endpoint)
const NO_STORE = { "Cache-Control": "no-store" };

export async function GET(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`customer:market-conditions:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/customer/market-conditions");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/market-conditions",
    ...
    return NextResponse.json(
      {
        ...
        message: isTimeout
          ...
          : "Market conditions service is temporarily unavailable.",
      },
