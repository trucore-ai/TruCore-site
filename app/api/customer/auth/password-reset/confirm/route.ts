/**
 * Password reset confirm proxy route.
 *
 * POST /api/customer/auth/password-reset/confirm - confirm password reset (unauthenticated, token-based)
 */
...
export async function POST(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`customer:auth:password-reset:confirm:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/auth/password-reset/confirm");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/auth/password-reset/confirm",
