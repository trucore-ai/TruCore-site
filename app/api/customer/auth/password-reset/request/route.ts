/**
 * Password reset request proxy route.
 *
 * POST /api/customer/auth/password-reset/request - request password reset (unauthenticated)
 */
...
const RATE_LIMIT_MAX = 5; // Tight limit for password reset requests
const MAX_BODY_BYTES = 4 * 1024;
const NO_STORE = { "Cache-Control": "no-store" };

export async function POST(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`customer:auth:password-reset:request:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/auth/password-reset/request");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "customer/auth/password-reset/request",
