...
export async function POST(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`customer:auth:login:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/auth/login");
  ...
  try {
    ...
    clearTimeout(timer);
    return NextResponse.json(
      { error: "upstream_unavailable", message: "Authentication service is temporarily unavailable." },
