...
export async function GET(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`onboarding:sample-intent:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/onboarding/sample-intent");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "onboarding/sample-intent",
    ...
    return NextResponse.json(
      { error: "upstream_unavailable", message: "Sample trade generation is temporarily unavailable." },
