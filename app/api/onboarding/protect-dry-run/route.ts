...
export async function POST(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`onboarding:protect-dry-run:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/onboarding/protect-dry-run");
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logSecurityEvent("customer_route_failure", {
        ip,
        meta: {
          route: "onboarding/protect-dry-run",
    ...
    return NextResponse.json(
      { error: "upstream_unavailable", message: "Protection simulation is temporarily unavailable." },
