...
export async function GET(req: NextRequest) {
  ...
  const rl = consumeRateLimit(`dashboard:activation:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
  ...
  const upstream = joinUpstreamUrl(getAtfApiBaseUrl(), "/dashboard/activation");
  ...
  try {
    ...
    return NextResponse.json(
      { error: "upstream_unavailable", message: "Activation service is temporarily unavailable." },
...
export async function POST(req: NextRequest) {
  const ip = getRequestIp(req);
  const rl = consumeRateLimit(`dashboard:activation:post:${sha256(ip)}`, {
    max: RATE_LIMIT_MAX,
    windowMs: 60_000,
  });
