...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .note{font-size:0.8rem;color:#64748b;line-height:1.45;margin-bottom:0.25rem}
    ...
    .divider{margin-top:1.25rem;padding-top:1.25rem;border-top:1px solid rgba(255,255,255,0.08)}
    .customer-link{font-size:0.8rem;color:#94a3b8;line-height:1.45}
    .customer-link a{color:#818cf8;text-decoration:none}
...
<body>
  <div class=\"card\">
    ...
    <p>This login is for authorized partners and operators only. Enter the signed access token that was issued to you by the TruCore team.</p>
    <p class=\"note\">Partner tokens start with <code style=\"color:#c4b5fd\">ptl_live_</code> and are delivered via email. API keys cannot be used here.</p>
    <form method=\"POST\" action=\"/portal/login\">
      <label for=\"token\">Signed access token</label>
      <input id=\"token\" name=\"token\" type=\"password\" required value=\"${escapedToken}\" autocomplete=\"off\" placeholder=\"ptl_live_…\" />
      <button type=\"submit\">Continue</button>
      ${errorHtml}
    </form>
    <div class=\"divider\">
      <p class=\"customer-link\">Looking for your customer dashboard? <a href=\"/login\">Sign in here</a> instead.</p>
    </div>
  </div>
</body>
</html>`;
